...
5. 在“设置”里点击“保存”后配置才会落盘
5. 回主界面后先选类型，再点“加载”
6. 状态栏出现“已加载类型”后再输入条件查询

高频误操作：
- 只切换类型但没点“加载”
- Excel 标题缺“/A”或“/B”，或顺序错误
- 在“设置”里改了内容但未点击“保存”就关闭
...
- Excel 标题必须包含“/A”和“/B”，且“/A”在前
- 可查询字段是“/A”和“/B”之间的列
- 支持两段模糊区间：`/C` 到 `/D`、`/E` 到 `/F`
- 若模糊区间存在：
	- 必须成对出现（`/C` 与 `/D`、`/E` 与 `/F`）
	- 必须位于“/A”和“/B”之间
	- 两段区间不能交叉
- 模糊区间内字段使用“包含匹配”，其他字段使用“精确匹配”
- 建议只通过程序“设置”修改 `types_config.json`
- 图标建议使用 PNG，推荐  256x256

补充说明：
- 匹配统一为大小写不敏感
- 结果区不显示关键列（`/A /B /C /D /E /F`）
...
Q2.1：为什么关闭“设置”会提示未保存更改？

只要你修改了标题，或执行了“新增/更新、删除、上移、下移”之一，就会进入待保存状态。点击“保存”后提示消失。
...
- 启动阶段：类型图标刷新做了轻量延迟，优先保证首屏可交互
- 结果渲染：列宽先快速估算，再空闲时补齐，兼顾速度与完整显示
- 查询执行：GUI 模式对可查询字段做了规范化缓存，重复查询更快
